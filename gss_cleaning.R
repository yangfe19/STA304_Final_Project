...
raw_data <- read_csv("AAItwurU.csv")
...
main_act <- raw_data %>% 
  mutate(main_activity = case_when(
    mpl_105a==1 ~ "Working at a paid job/business",
    mpl_105b==1 ~ "Looking for paid work",
    mpl_105c==1 ~ "Going to school",
    mpl_105d==1 ~ "Caring for children",
    mpl_105e==1 ~ "Household work", 
    mpl_105i==1 ~ "Other", 
    TRUE~ "NA")) %>% 
...
remove(cw_statements,
       labels_raw,raw_data
       ,variable_descriptions
       ,age_diff,dict,
       labels_raw_tibble,
       main_act,add_cw_text)
